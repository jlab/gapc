...
grammar gra_nodangle uses sig_foldrna(axiom = struct) {
  ...
  weak      = hl(BASE,                          REGION with minsize(3),          BASE) with basepairing    |
              bl(BASE, REGION with maxsize(30), weak,                          BASE) with basepairing      | 
              il(BASE, REGION with maxsize(30), weak, REGION with maxsize(30), BASE) with basepairing      
              # h;

}

instance ins_mfe = gra_nodangle(alg_mfe);
instance ins_pfunc = gra_nodangle(alg_pfunc);
instance ins_count = gra_nodangle(alg_count);
instance ins_enum = gra_nodangle(alg_enum);
