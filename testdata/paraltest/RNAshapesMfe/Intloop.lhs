> module RNAshapesMfe.Intloop where
...
> pairlist:: [(Ebase,Ebase)]
> pairlist = [(C,G),(G,C),(G,U),(U,G),(A,U),(U,A)]

> pairs:: (Ebase, Ebase) -> Int
> pairs (C,G) = 0
> pairs (G,C) = 1
> pairs (G,U) = 2
> pairs (U,G) = 3
...
> il11_energy inp lb rb = getintloop11 (inp!(lb)) (inp!(lb+1)) (inp!(lb+2)) (inp!(rb-2)) (inp!(rb-1)) (inp!(rb))

> getintloop11:: Num a => Ebase -> Ebase -> Ebase -> Ebase -> Ebase -> Ebase -> a 
> getintloop11 a b c d e f
>	       | b==N      = 180
>	       | e==N      = 180
>	       | otherwise = arrayintloop11!(pairs (a,f), pairs (c,d), basemap b, basemap e)

 store all data in a 4 dimensional array. The first two indices run over pairs
 Last two indices run over A,C,G,U

> arrayintloop11:: ( Num b) => Array (Int,Int,Int,Int) b
> arrayintloop11 = array ((0,0,0,0),(5,5,3,3)) 
>			 [((pairs (i,n), pairs (k,l), basemap j, basemap m), intloop11 i j k l m n)|
>				  (i,n) <- pairlist, (k,l) <- pairlist,
>				      j <- [A,C,G,U],    m <- [A,C,G,U]]
