...
#ifndef RTLIB_ADP_SPECIALIZATION_PARETO_3_YUKISH_STEP_HH_
#define RTLIB_ADP_SPECIALIZATION_PARETO_3_YUKISH_STEP_HH_
...
typename yp_in_list<yp_split<T> >::iterator yp_sortedSplitMarryp_inner1(
  ...
  Compare &c, int d, int dim, T &median) {
    if (ob.begin == ob.end) {
        return in;
    }
...
typename yp_in_list<yp_split<T> >::iterator yp_sortedSplitMarryp_inner3(
  yp_in_list<yp_split<T> > &splits,
  typename yp_in_list<yp_split<T> >::iterator in , yp_split<T> ob, Compare &c,
  int d, int dim, const T &median) {
    if (ob.begin == ob.end) {
        return in;
    }
...
template<class T, typename Compare>
void yp_marryBrute(
  yp_list<T>  &answers, const yp_split<T> &x, yp_list<T> &y, Compare &c,
  int s, int dim, const bool keep_equal) {
    for (typename yp_list<T>::iterator el_x = x.begin; el_x != x.end; ++el_x) {
        bool add = true;
...
void yp_marry_base(
  ...
  int dim, int blocksize) {
    ...
    for (; s_x != raw_splits_x.end() &&
         s_y != raw_splits_y.end(); ++s_x, ++s_y) {
...
template<class T, typename Compare, typename Sorter>
void yp_marry_base(
  yp_list<T> &answers , yp_list<T> &x, yp_list<T> &y, Compare &c,
  Sorter &sort, int s, int dim, int blocksize) {
    ...
    while (splits_x.size() > 1) {
        ...
        for (; s_x != splits_x.end() && s_y != splits_y.end(); ++s_x, ++s_y) {
            ...
            if ((*s_x)->depth == (*snext_x)->depth) {
                ...
                yp_marry_base(
                  tmp->list, (*s_x)->list, (*snext_y)->list.begin(),
...
inline void yp_marry(
  ...
  Compare &c, Sorter &sort, int dim, int blocksize, const bool keep_equal) {
    yp_marry(
      answers_x, answers_y, x.begin(), x.end(), y.begin(), y.end(), c,
      sort, dim, blocksize, keep_equal);
}
...
template<class T, typename Compare, typename Sorter>
inline void append(
  List_Ref<T> &answers, T &in, Compare &c, Sorter &s, const bool keep_equal) {
    const int dim = c.dim;
    bool add = true;
...
inline void append(
  ...
  if (isEmpty(answers)) {
       _MOVE_RANGE(
         inserts.ref().begin(), inserts.ref().end(), std::back_inserter(
           answers.ref()));
      return;
  }
  ...
  for (typename List_Ref<T>::iterator it = answers.ref().begin();
       it != answers.ref().end(); ++it) {
      copy_x.push_back(&(*it));
  }
  for (typename List_Ref<T>::iterator it = inserts.ref().begin();
       it != inserts.ref().end(); ++it) {
      copy_y.push_back(&(*it));
  }
  ...
  yp_marry(
    answers_x, answers_y, copy_x, copy_y, c, s, c.dim, yukish_cutoff,
    keep_equal);

  List_Ref<T> newans;
  for (typename yp_list<T>::iterator ans = answers_x.begin();
       ans != answers_x.end(); ++ans) {
       push_back(newans, _MOVE(**ans));
  }
  for (typename yp_list<T>::iterator ans = answers_y.begin();
       ans != answers_y.end(); ++ans) {
       push_back(newans, _MOVE(**ans));
  }
...
#endif  // RTLIB_ADP_SPECIALIZATION_PARETO_3_YUKISH_STEP_HH_
