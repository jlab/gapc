...
inline uint32_t hashable_value(int t) {
  return t;
}

namespace Hash {
// FIXME
inline uint32_t hashable_value(size_t t) {
  return t;
}
...
template <typename T> struct Size2pow {
  T initial() const {
    // return 512;
    // return 4;
    // return 8;
...
template <typename T, typename U = uint32_t> struct Default_Inspector {
  T init(const T &x) const {
    return x;
  }
 ...
 public:
  ...
  void filter() {
    ...
    if (inspector.filter()) {
      ...
      swap(init, b);
      used_ = 0;
      for (typename Vector_Sparse<T, U>::iterator i = a.begin(); i != a.end();
           ++i) {
...
template<SET_TEMPLATE_DECL> struct Set_Dummy {
  static Pool<Set<SET_TEMPLATE_ARGS> > pool;
};

template<SET_TEMPLATE_DECL> Pool<Set<SET_TEMPLATE_ARGS> >
...
template<class T, class I> class Ref : public ::Ref::Lazy<Set<T, I> > {
 private:
 public:
};
...
template<class T> inline void finalize(T &x) {
}

template<class T, class I> inline void finalize(Hash::Ref<T, I> &x) {
  x->finalize();
}

template<class T, class I> inline
void push_back(Hash::Ref<T, I> &x, const T &e) {
  assert(is_not_empty(e));
  x->add(e);
}

template<class T, class I> inline
void append(Hash::Ref<T, I> &x, Hash::Ref<T, I> &e) {
  if (isEmpty(e)) {
    return;
  }
  assert(&x.ref() != &e.ref());
...
template<class T, class I, typename Iterator> inline
void append_filter(Hash::Ref<T, I> &x, std::pair<Iterator, Iterator> i) {
...
template<class T, class I> inline void empty(Hash::Ref<T, I> &x) {
}

template<class T, class I> inline bool isEmpty(const Hash::Ref<T, I> &x) {
  return !x.l || x.const_ref().isEmpty();
}

template<class T, class I> inline void erase(Hash::Ref<T, I> &x) {
}

template<class T, class U> inline void update_filter(T &x, const U &a) {
  x.update(a);
}
