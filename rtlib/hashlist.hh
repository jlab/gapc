...
class Hash_List_Back {
 ...
 public:
  ...
  Hash_List_Back() : size(0), ref_count(1) {
  }
  ...
  iterator begin() const {
    return list.const_ref().begin();
  }
  iterator end() const {
    return list.const_ref().end();
  }
}
...
template<class T, class I> inline void empty(Hash_List<T, I> &x) {
}

template<class T, class I> inline bool isEmpty(const Hash_List<T, I> &x) {
  return !x.l || x.const_ref().isEmpty();
}

template<class T, class I> inline void erase(Hash_List<T, I> &x) {
}
