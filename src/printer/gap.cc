...
void Printer::GapPrinter::print(AST* ast) {
  ...
  for (hashtable<std::string, Algebra*>::iterator i = ast->algebras.begin();
       i != ast->algebras.end(); i++) {
  ...
  for (hashtable<std::string, Instance*>::iterator i = ast->instances.begin();
       i != ast->instances.end(); i++) {
    std::string instanceName = (*i).first;
...
void Printer::GapPrinter::print(std::list<Import*> imports) {
  for (std::list<Import*>::iterator i = imports.begin();
       i != imports.end(); i++) {
    print(*i);
  }
}
...
void Printer::GapPrinter::print(hashtable<std::string, Type::Base*> types) {
  for (hashtable<std::string, Type::Base*>::iterator i = types.begin();
       i != types.end(); i++) {
...
void Printer::GapPrinter::print(Signature* sig) {
  ...
  bool firstLoopRun = true;
  for (hashtable <std::string, Arg*>::iterator i = sig->args.begin();
       i != sig->args.end(); i++) {
    if (!firstLoopRun) {
      oStream << ", ";
    }
  ...
  for (hashtable <std::string, Fn_Decl*>::iterator i = sig->decls.begin();
       i != sig->decls.end(); i++) {
  ...
  for (hashtable <std::string, Fn_Decl*>::iterator i = sig->choice_fns.begin();
       i != sig->choice_fns.end(); i++) {
    oStream << indention();
      print((*i).second);
      oStream << ";" << std::endl;
  }
...
void Printer::GapPrinter::print(Algebra* alg) {
  ...
  oStream << "algebra " << *alg->name << " implements "
    << (alg->signature_name == NULL ? "" : *alg->signature_name);

  // print the algebra parameters
  oStream << " (";
  bool firstLoopRun = true;
  for (hashtable<std::string, Type::Base*>::iterator i = alg->params.begin();
       i != alg->params.end(); i++) {
    if (!firstLoopRun) {
      oStream << ", ";
    }
  ...
  for (hashtable<std::string, Fn_Def*>::iterator i = alg->fns.begin();
       i != alg->fns.end(); i++) {
...
void Printer::GapPrinter::print(Grammar* grammar) {
  oStream << indention() << "grammar " << *grammar->name << " uses "
    << *grammar->sig_name << " (axiom = " << *grammar->axiom_name << ") {"
    << std::endl;
  incIndention();
  ...
  if (grammar->tabulated.size() > 0) {
    oStream << "tabulated {";
    bool firstLoopRun = true;
    for (hashtable<std::string, Symbol::NT*>::iterator i =
        grammar->tabulated.begin(); i != grammar->tabulated.end(); i++) {
      if (!firstLoopRun) {
        oStream << ", ";
      }
      oStream << (*i).first;
  ...
  for (hashtable<std::string, Symbol::Base*>::iterator i = grammar->NTs.begin();
    ...
    if ((*i).second->is(Symbol::TERMINAL)) {
      Symbol::Terminal* terminal =
        dynamic_cast<Symbol::Terminal*> ((*i).second);
...
void Printer::GapPrinter::print(Instance* instance) {
  oStream << "instance " << *instance->name() << " = "
    << *instance->grammar()->name << " (";
...
void Printer::GapPrinter::print(Para_Decl::Multi* m) {
  ...
  bool firstLoopRun = true;
  for (std::list<Para_Decl::Simple*>::iterator i = paras.begin();
       i != paras.end(); i++) {
    if (!firstLoopRun) {
      oStream << ", ";
    }
...
void Printer::GapPrinter::print(Alt::Simple* s) {
  ...
  if (s->args.size() > 0) {
    oStream << " (";
    bool firstLoopRun = true;
    for (std::list<Fn_Arg::Base*>::iterator i = s->args.begin();
         i != s->args.end(); i++) {
      if (!firstLoopRun) {
        oStream << ", ";
      }
...
void Printer::GapPrinter::print(Alt::Block* b) {
  oStream << "(";
  bool firstLoopRun = true;
  for (std::list<Alt::Base*>::iterator i = b->alts.begin();
       i != b->alts.end(); i++) {
    if (!firstLoopRun) {
      oStream << " | ";
    }
...
void Printer::GapPrinter::print(std::list<Filter*> fs) {
  for (std::list<Filter*>::iterator i = fs.begin(); i != fs.end(); i++) {
    ...
    oStream << "(";
    bool firstLoopRun = true;
    for (std::list<Expr::Base*>::iterator j = filter->args.begin();
         j != filter->args.end(); j++) {
      if (!firstLoopRun) {
        oStream << ", ";
      }
...
void Printer::GapPrinter::print(std::list<Statement::Base*> b) {
  for (std::list<Statement::Base*>::iterator i = b.begin();
       i != b.end(); i++) {
    print(*i);
  }
}
...
void Printer::GapPrinter::print(Statement::Fn_Call* f) {
  ...
  bool firstLoopRun = true;
  for (std::list<Expr::Base*>::iterator i = f->args.begin();
       i != f->args.end(); i++) {
    if (!firstLoopRun) {
      oStream << ", ";
    }
...
void Printer::GapPrinter::print(Fn_Decl* fnDecl) {
  ...
  oStream << " " << *fnDecl->name << " (";
  bool firstLoopRun = true;
  for (std::list<Type::Base*>::iterator i = fnDecl->types.begin();
       i != fnDecl->types.end(); i++) {
    if (!firstLoopRun) {
      oStream << ", ";
    }
    print(*i);
