...
bool Options::check()
{
	...
	if (classified && (backtrack || subopt ) )
		Log::instance()->error("Can't combine --subopt-classify with --backtrace/subopt options");
	
	if (!instance.empty() && !product.empty())
		Log::instance()->error("Can't combine --instance with --product");
	
	if (window_mode && cyk)
		Log::instance()->error("Currently --window-mode is just possible without --cyk.");
